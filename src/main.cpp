...
#if defined(NDEBUG)
# error "Circulatecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Circulatecoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Circulatecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Circulatecoin: find block wanted by given orphan block
...
{
    int64_t nSubsidy = 0.57 * COIN;

    if(pindexPrev->nHeight > 10555)
        nSubsidy = 0.5707 * COIN;
...
// Circulatecoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Circulatecoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Circulatecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Circulatecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Circulatecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Circulatecoin: track money supply and mint amount info
...
// Circulatecoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Circulatecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Circulatecoin: compute stake entropy bit for stake modifier
    ...
    // Circulatecoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Circulatecoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Circulatecoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Circulatecoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Circulatecoin-loadblk");
